...
-- 3. CALL PARTICIPANTS TABLE (Supports 1:1 and Multi-user Room Calls)
...
-- 5. HIGH-PERFORMANCE B-TREE INDEXES FOR SUB-MILLISECOND SIGNAL DISPATCH
...
-- Helper security function to verify call membership
...
CREATE POLICY "Call members can view call sessions" ON public.call_sessions
    FOR SELECT
    USING (
        caller_id = auth.uid() 
        OR auth.uid() IS NULL 
        OR public.is_call_member(id, auth.uid())
    );

DROP POLICY IF EXISTS "Authenticated users can initiate call sessions" ON public.call_sessions;
CREATE POLICY "Authenticated users can initiate call sessions" ON public.call_sessions
    FOR INSERT
    WITH CHECK (
        caller_id = auth.uid() OR auth.uid() IS NULL
    );

DROP POLICY IF EXISTS "Call members can update call sessions" ON public.call_sessions;
CREATE POLICY "Call members can update call sessions" ON public.call_sessions
    FOR UPDATE
    USING (
        caller_id = auth.uid() 
        OR auth.uid() IS NULL 
...
-- RLS: CALL PARTICIPANTS
DROP POLICY IF EXISTS "Participants can view their call members" ON public.call_participants;
CREATE POLICY "Participants can view their call members" ON public.call_participants
    FOR SELECT
    USING (
        user_id = auth.uid() 
        OR auth.uid() IS NULL 
        OR public.is_call_member(call_id, auth.uid())
    );

DROP POLICY IF EXISTS "Call initiators and participants can insert participants" ON public.call_participants;
CREATE POLICY "Call initiators and participants can insert participants" ON public.call_participants
    FOR INSERT
    WITH CHECK (
        auth.uid() IS NULL 
        OR public.is_call_member(call_id, auth.uid()) 
...
CREATE POLICY "Participants can update their own call state" ON public.call_participants
    FOR UPDATE
    USING (
        user_id = auth.uid() 
        OR auth.uid() IS NULL 
...
-- RLS: CALL SIGNALS (Strict peer isolation)
DROP POLICY IF EXISTS "Only target receivers and senders can view signals" ON public.call_signals;
CREATE POLICY "Only target receivers and senders can view signals" ON public.call_signals
    FOR SELECT
    USING (
        auth.uid() IS NULL
        OR sender_id = auth.uid()
        OR receiver_id = auth.uid()
        OR (receiver_id IS NULL AND public.is_call_member(call_id, auth.uid()))
    );

DROP POLICY IF EXISTS "Call members can send signaling messages" ON public.call_signals;
CREATE POLICY "Call members can send signaling messages" ON public.call_signals
    FOR INSERT
    WITH CHECK (
        auth.uid() IS NULL
        OR (sender_id = auth.uid() AND public.is_call_member(call_id, auth.uid()))
    );

-- 7. TRIGGERS & AUTOMATION

-- A. Automatically update call duration and timestamps
...
BEGIN
    ...
    -- Set connected_at when state becomes active
...
CREATE TRIGGER tr_call_session_lifecycle
    BEFORE UPDATE ON public.call_sessions
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_call_session_lifecycle();

-- B. Automatic TTL cleanup of transient signaling messages after call ends
...
CREATE TRIGGER tr_cleanup_call_signals
    AFTER UPDATE ON public.call_sessions
    FOR EACH ROW
    WHEN (NEW.status IN ('ended', 'missed', 'rejected', 'busy'))
    EXECUTE FUNCTION public.cleanup_finished_call_signals();

-- 8. HIGH-CONCURRENCY RPC STORED PROCEDURES
...
BEGIN
    ...
    -- Check if any single recipient is already in an active call
    IF NOT p_is_group AND array_length(p_receiver_ids, 1) = 1 THEN
        SELECT EXISTS (
            SELECT 1 FROM public.call_participants cp
            JOIN public.call_sessions cs ON cs.id = cp.call_id
            WHERE cp.user_id = p_receiver_ids[1]
              AND cs.status IN ('initiating', 'ringing', 'active')
        ) INTO v_is_busy;
        ...
        IF v_is_busy THEN
            RETURN jsonb_build_object(
                'success', false,
                'error', 'USER_BUSY',
                'message', 'Le correspondant est déjà en ligne sur un autre appel'
            );
        END IF;
    END IF;
    ...
    INSERT INTO public.call_sessions (
        caller_id, call_type, status, is_group, room_id, metadata
    ) VALUES (
        v_caller_id, p_call_type, 'ringing', p_is_group, v_room_id, p_metadata
    ) RETURNING id INTO v_call_id;

    -- Insert Caller as joined participant
    INSERT INTO public.call_participants (
        call_id, user_id, status, joined_at
    ) VALUES (
        v_call_id, v_caller_id, 'joined', NOW()
    );

    -- Insert Recipients as ringing participants
    FOREACH v_receiver_id IN ARRAY p_receiver_ids LOOP
        INSERT INTO public.call_participants (
            call_id, user_id, status
        ) VALUES (
            v_call_id, v_receiver_id, 'ringing'
        );
        ...
        INSERT INTO public.notifications (
            ...
            v_receiver_id,
            v_caller_id,
            CASE WHEN p_call_type = 'video' THEN 'incoming_call_video' ELSE 'incoming_call_audio' END,
            v_call_id
        );
    END LOOP;
...
-- C. End or Reject Call RPC
CREATE OR REPLACE FUNCTION public.end_call(
    p_call_id UUID,
    p_reason TEXT DEFAULT 'normal_hangup'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    UPDATE public.call_participants
    SET status = CASE WHEN v_call.status = 'ringing' AND v_user_id != v_call.caller_id THEN 'declined' ELSE 'left' END,
        left_at = NOW(),
        updated_at = NOW()
    WHERE call_id = p_call_id AND user_id = v_user_id;
    ...
    WHERE call_id = p_call_id AND status = 'joined';

    IF NOT v_call.is_group OR v_active_count <= 1 OR v_user_id = v_call.caller_id THEN
        UPDATE public.call_sessions
        SET status = CASE 
                WHEN v_call.status = 'ringing' AND v_user_id != v_call.caller_id THEN 'rejected'
                WHEN v_call.status = 'ringing' AND v_user_id = v_call.caller_id THEN 'missed'
                ELSE 'ended'
            END,
            ended_at = NOW(),
            end_reason = p_reason,

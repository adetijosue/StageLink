      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ fontSize: '1.1rem', fontWeight: 900, margin: 0 }}>{language === 'en' ? 'Musical Resume & Press Kit' : 'CV Musical & Press Kit'}</h3>
          </div>
          <button
            onClick={onClose}
            style={{
        ...
        <div style={{ display: 'flex', background: isDarkMode ? '#1E293B' : '#F1F5F9', padding: 4, borderRadius: '14px', marginBottom: '20px' }}>
          <button onClick={() => setActiveTab('interactive')} style={{ flex: 1, padding: '10px', borderRadius: '11px', border: 'none', background: activeTab === 'interactive' ? '#0066FF' : 'transparent', color: activeTab === 'interactive' ? '#FFF' : '#64748B', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>{language === 'en' ? 'Artist Profile' : 'Fiche Artiste'}</button>
          <button onClick={() => setActiveTab('file')} style={{ flex: 1, padding: '10px', borderRadius: '11px', border: 'none', background: activeTab === 'file' ? '#0066FF' : 'transparent', color: activeTab === 'file' ? '#FFF' : '#64748B', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>{language === 'en' ? 'PDF File' : 'Fichier PDF'}</button>
        </div>
          ...
          {activeTab === 'interactive' ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
               ...
               <div style={{ background: isDarkMode ? '#1E293B' : '#F8FAFC', padding: '15px', borderRadius: '16px' }}>
                  <h4 style={{ fontSize: '0.75rem', fontWeight: 900, color: '#64748B', marginBottom: '8px' }}>{language === 'en' ? 'INSTRUMENTS & SKILLS' : 'INSTRUMENTS & SKILLS'}</h4>
               ...
               <div style={{ background: isDarkMode ? '#1E293B' : '#F8FAFC', padding: '15px', borderRadius: '16px' }}>
                  <h4 style={{ fontSize: '0.75rem', fontWeight: 900, color: '#64748B', marginBottom: '8px' }}>{language === 'en' ? 'BIOGRAPHY' : 'BIOGRAPHIE'}</h4>
                  <p style={{ fontSize: '0.85rem', lineHeight: 1.5, margin: 0 }}>{user.bio || (language === 'en' ? 'Not specified.' : 'Non renseignée.')}</p>
               </div>
            </div>
          ) : (
            <div
            ...
            >
              ...
              <div style={{ fontWeight: 800, fontSize: '0.9rem' }}>{isOwnProfile ? (language === 'en' ? 'Upload my Musical Resume' : 'Importer mon CV Musical') : (language === 'en' ? 'No file' : 'Aucun fichier')}</div>
              <p style={{ fontSize: '0.75rem', color: '#64748B', marginTop: '5px' }}>{language === 'en' ? 'Accepts PDF, DOCX and JPG files' : 'Accepte les fichiers PDF, DOCX et JPG'}</p>
        ...
        >
          <Download size={18} style={{ marginRight: 8 }} /> {language === 'en' ? 'Download as PDF' : 'Télécharger en PDF'}
        </button>
      </div>
    </div>
  );
